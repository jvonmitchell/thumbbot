    ...
    Thumbbot.prototype.handleImage = function(srcPath, destPath, callback) {
      ...
      if (this.options.method === 'resize') {
        image.size(function (err,size){
          if( (toWidth && size.width>toWidth) || (toHeight && size.height>toHeight) ) {
            //We need to resize
            console.log("We need to resize");
            console.log(toWidth,toHeight,size.width,size.height);
            image.resize(toWidth,toHeight).write(destPath,function (err) {
             if(callback) {
              callback();
             }
            });
